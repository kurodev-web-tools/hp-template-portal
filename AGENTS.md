...
- For template renewals, create one worktree per template and keep that worktree dedicated to that template until the renewal is finished.
...
## Renewal Execution
- For renewal work, the worktree creation and the implementation loop are owned by Codex, while the final `push` and `PR` step is owned by the user.
- Progress renewal work in meaningful units, confirm each unit before moving on, and commit per completed unit.
- Keep the first phase of a renewal to structure only: layout, section order, and core containers without motion or decorative effects.
