document.addEventListener('DOMContentLoaded', () => {
    ...
    const applyTheme = (isDark) => {
        ...
        localStorage.setItem('knowledge-theme', isDark ? 'dark' : 'light');
    ...
    applyTheme(localStorage.getItem('knowledge-theme') === 'dark');

    menuToggle?.addEventListener('click', toggleMenu);
    backdrop?.addEventListener('click', closeMenu);
    menuLinks.forEach((link) => link.addEventListener('click', closeMenu));
    themeToggles.forEach((button) => button.addEventListener('click', () => applyTheme(!root.classList.contains('dark'))));
