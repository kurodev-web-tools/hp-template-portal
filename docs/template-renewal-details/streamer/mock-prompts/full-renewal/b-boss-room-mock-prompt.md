# Streamer B: Boss Room Mock Prompt

## Purpose
- このファイルは streamer/b の first viewport mock 画像を作るためのプロンプト。
- `design.md` 形式の考え方に合わせ、YAML design tokens + human-readable rationale + image prompt に分ける。
- Source detail: `docs/template-renewal-details/streamer/full-renewal/b-boss-room.md`
...
description: "RPGボス系VTuber向けの公式サイト first viewport mock。スケジュール、切り抜き、ファンコミュニティへつなぐ。"
colors:
  primary: "#d4af37"
  secondary: "#8a6d3b"
  surface: "#121212"
...
## Overview
- Target audience: RPGボス系VTuber
- Conversion goal: スケジュール、切り抜き、ファンコミュニティ
...
## Colors
- Primary: `#d4af37` を世界観の主役色、Live状態、重要アクセントに使う。
- Secondary: `#8a6d3b` をUI枠、背景レイヤー、補助アクセントに使う。
- Surface: `#121212` を背景/カード面に使う。
...
- Information placement: side rail for navigation, lower-left for status/Follow, mid-right for Next Raid/Schedule, lower-right for Contract/Contact.
...
- Implementation hint: CSS grid, side rail, bordered panels, HP bars, and static throne background layers.
...
## Components
- Color system: #d4af37, #8a6d3b, #121212 を主役色、CTA色、背景演出色に分ける。Live/Follow/Contact は色だけでなくラベルでも区別する。
...
- Signature motion: Boss Room の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
Theme: "Boss Room".
Audience: RPGボス系VTuber.
Visual direction: 玉座/ステータスUI、重厚な暗金、HPバー型セクション.
...
- No generic top navigation with identical right-side stacked cards, no light top header, no balanced two-column card grid, no pastel floating cards.
...
- The visual identity matches "Boss Room" and still feels like a usable website.
