# Streamer L: Lunar Phase Mock Prompt

## Purpose
- このファイルは streamer/l の first viewport mock 画像を作るためのプロンプト。
- `design.md` 形式の考え方に合わせ、YAML design tokens + human-readable rationale + image prompt に分ける。
- Source detail: `docs/template-renewal-details/streamer/full-renewal/l-lunar-phase.md`
...
Imagery direction: 月相ダイヤル、夜空、静かな天文UI、月明かり。配信予定は月齢カレンダーとして配置する。
...
- No dense dashboard, no cyber terminal, no idol ticket strip, no identical right-side stacked cards.
...
- The visual identity matches "Lunar Phase" and still feels like a usable website.
