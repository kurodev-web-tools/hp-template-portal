# Streamer M: Metallic Chrome Mock Prompt

## Purpose
- このファイルは streamer/m の first viewport mock 画像を作るためのプロンプト。
- `design.md` 形式の考え方に合わせ、YAML design tokens + human-readable rationale + image prompt に分ける。
- Source detail: `docs/template-renewal-details/streamer/full-renewal/m-metallic-chrome.md`
...
---
version: "alpha"
name: "Streamer M: Metallic Chrome"
description: "近未来/ファッション系向けの公式サイト first viewport mock。最新配信、ビジュアルギャラリー、案件へつなぐ。"
colors:
  primary: "#9ca3af"
  secondary: "#111827"
  surface: "#000000"
...
## Overview
- Target audience: 近未来/ファッション系
- Conversion goal: 最新配信、ビジュアルギャラリー、案件
...
## Colors
- Primary: `#9ca3af` を世界観の主役色、Live状態、重要アクセントに使う。
- Secondary: `#111827` をUI枠、背景レイヤー、補助アクセントに使う。
...
- Information placement: center chrome hero pedestal, left spec/navigation strip, right live demo/schedule display, bottom polished contact dock.
...
- Implementation hint: reflective panels, pedestal layout, spec strips, demo card, and restrained gradients.
...
## Components
- Color system: #9ca3af, #111827, #000000 を主役色、CTA色、背景演出色に分ける。Live/Follow/Contact は色だけでなくラベルでも区別する。
...
- Signature motion: Metallic Chrome の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
Theme: "Metallic Chrome".
Audience: 近未来/ファッション系.
Visual direction: クローム反射、雑誌的Hero、UIは整理.
Imagery direction: クロームのショールーム、反射する金属パネル、センター展示台、プロダクト発表風の情報配置。
...
- No fantasy UI, no pastel prism layout, no generic corporate SaaS dashboard, no repeated right-side card stack.
...
- The visual identity matches "Metallic Chrome" and still feels like a usable website.
