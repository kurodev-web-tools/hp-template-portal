# Streamer U: Urban Graffiti Mock Prompt

## Purpose
- このファイルは streamer/u の first viewport mock 画像を作るためのプロンプト。
- `design.md` 形式の考え方に合わせ、YAML design tokens + human-readable rationale + image prompt に分ける。
- Source detail: `docs/template-renewal-details/streamer/full-renewal/u-urban-graffiti.md`
...
---
version: "alpha"
name: "Streamer U: Urban Graffiti"
description: "ストリート/ラップ/カルチャー系向けの公式サイト first viewport mock。配信/ライブ予定、SNS、コラボ相談へつなぐ。"
colors:
  primary: "#fbbf24"
  secondary: "#262626"
  surface: "#181818"
...
## Overview
- Target audience: ストリート/ラップ/カルチャー系
- Conversion goal: 配信/ライブ予定、SNS、コラボ相談
...
## Colors
- Primary: `#fbbf24` を世界観の主役色、Live状態、重要アクセントに使う。
- Secondary: `#262626` をUI枠、背景レイヤー、補助アクセントに使う。
- Surface: `#181818` を背景/カード面に使う。
...
- Information placement: wall-tag header, center mural hero, torn-poster Live/Schedule, sticker Follow icons, lower-right booking/contact poster.
...
- Implementation hint: poster collage grid, sticker CTAs, wall texture, torn paper panels, and readable labels.
...
## Components
- Color system: #fbbf24, #262626, #181818 を主役色、CTA色、背景演出色に分ける。Live/Follow/Contact は色だけでなくラベルでも区別する。
...
- Signature motion: Urban Graffiti の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
Audience: ストリート/ラップ/カルチャー系.
Visual direction: 壁面グラフィティ、イベント告知、太いCTA.
Imagery direction: ストリート壁面、ステッカー、ポスター、落書きタグ、イベント告知。情報は貼り紙/ステッカーとして配置する。
...
- No polished corporate grid, no fantasy UI, no pastel prism panels, no identical right-side card stack.
...
- The visual identity matches "Urban Graffiti" and still feels like a usable website.
