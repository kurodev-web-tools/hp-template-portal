# Streamer W: Wide Pan Mock Prompt

## Purpose
- このファイルは streamer/w の first viewport mock 画像を作るためのプロンプト。
- `design.md` 形式の考え方に合わせ、YAML design tokens + human-readable rationale + image prompt に分ける。
- Source detail: `docs/template-renewal-details/streamer/polish/w-wide-pan.md`
...
Imagery direction: 横長シネマフレーム、配信シーンのパノラマ、タイムライン帯。カードを積まず横方向に情報を流す。
...
- No vertical card stack, no dense dashboard, no idol ticket strip, no generic hero-left/cards-right layout.
...
- The visual identity matches "Wide Pan" and still feels like a usable website.
