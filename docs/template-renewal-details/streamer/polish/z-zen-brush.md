# Streamer Z: Zen Brush Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_z
- Existing path: `templates/streamer/z`
- Current theme name: Zen Brush
- Renewal decision: Polish
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Zen Brush」。features は Zen, Brush Stroke, Multi-page、主要色は #171717, #fafafa, #404040。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 和/書/落ち着いた配信者
- Conversion goal: 配信予定、作品/アーカイブ、SNS
- Core concept: 和/書/落ち着いた配信者 向けの公式サイト。既存の Zen Brush らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Hero (既存維持 + 改善): 初見が誰のサイトか即理解する。入れる内容: キャラ名、ジャンル、Live状態、SNS CTA。注意: 世界観演出で文字を読めなくしない。
- Profile (既存維持 + 改善): 配信人格と活動ジャンルを伝える。入れる内容: タグ、活動時間、好きな企画、プラットフォーム。注意: 設定説明だけで終わらせない。
- Schedule (既存維持 + 改善): 次に何を見ればいいか示す。入れる内容: 次回配信、週間枠、通知CTA。注意: 装飾より日時の読みやすさ優先。
- Clips / Archive (既存維持 + 改善): 魅力を短時間で伝える。入れる内容: 代表動画、人気企画、切り抜き導線。注意: iframe多用で重くしない。
- Community / Contact (既存維持 + 改善): ファン参加と案件相談を分ける。入れる内容: Discord/X/ハッシュタグ/Business contact。注意: ファン向けCTAと企業向けCTAを混ぜない。

## デザイン / レイアウト方針
- Layout base: PC は世界観UIを強め、SP は配信予定とSNS導線を最短距離にする。
- Visual direction: 筆跡、余白、静かなCTA
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #171717, #fafafa, #404040 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #171717, #fafafa, #404040 を主役色、CTA色、背景演出色に分ける。Live/Follow/Contact は色だけでなくラベルでも区別する。
...
- Audience language: 和/書/落ち着いた配信者 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-z&plan=standard` へ接続する想定。
...
- Signature motion: Zen Brush の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
