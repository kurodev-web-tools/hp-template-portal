# Streamer X: Xtreme Action Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_x
- Existing path: `templates/streamer/x`
- Current theme name: Xtreme Action
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Xtreme Action」。features は Xtreme, Shockwave, Snap Scroll、主要色は #fbbf24, #000000, #78350f。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: スポーツ/アクション配信者
- Conversion goal: 大会予定、配信、スポンサー相談
- Core concept: スポーツ/アクション配信者 向けの公式サイト。既存の Xtreme Action らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Color: 既存の #fbbf24, #000000, #78350f を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #fbbf24, #000000, #78350f を主役色、CTA色、背景演出色に分ける。Live/Follow/Contact は色だけでなくラベルでも区別する。
...
- Audience language: スポーツ/アクション配信者 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-x&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Xtreme Action のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
- Signature motion: Xtreme Action の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Xtreme Action」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
