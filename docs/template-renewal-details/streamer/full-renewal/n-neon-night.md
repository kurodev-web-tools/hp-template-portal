# Streamer N: Neon Night Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_n
- Existing path: `templates/streamer/n`
...
## 現状の読み取り
既存テーマは「Neon Night」。features は Neon, Urban, Street、主要色は #7000ff, #1e1b4b, #020617。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 夜遊び/クラブ配信者
- Conversion goal: イベント配信、告知、コミュニティ
- Core concept: 夜遊び/クラブ配信者 向けの公式サイト。既存の Neon Night らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Color: 既存の #7000ff, #1e1b4b, #020617 を起点に、CTA色と装飾色を分ける。
...
- Audience language: 夜遊び/クラブ配信者 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-n&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Neon Night のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
- Signature motion: Neon Night の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Neon Night」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
