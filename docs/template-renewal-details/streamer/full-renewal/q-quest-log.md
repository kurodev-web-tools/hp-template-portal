# Streamer Q: Quest Log Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_q
- Existing path: `templates/streamer/q`
- Current theme name: Quest Log
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Quest Log」。features は RPG UI, Log Style, Fantasy、主要色は #f5e6be, #5d4037, #2b1d0e。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: RPG/企画配信者
- Conversion goal: 参加企画、次回配信、コミュニティ
- Core concept: RPG/企画配信者 向けの公式サイト。既存の Quest Log らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Color: 既存の #f5e6be, #5d4037, #2b1d0e を起点に、CTA色と装飾色を分ける。
...
- Audience language: RPG/企画配信者 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-q&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Quest Log のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Quest Log」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
