# Streamer R: Rogue Stealth Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_r
- Existing path: `templates/streamer/r`
- Current theme name: Rogue Stealth
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Rogue Stealth」。features は Stealth, Smoke Effect, Dark Red、主要色は #991b1b, #000000, #1a1a1a。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: ステルス/スパイ系
- Conversion goal: 配信予定、秘密任務風企画、SNS
- Core concept: ステルス/スパイ系 向けの公式サイト。既存の Rogue Stealth らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Visual hook: 任務ブリーフィング、暗いUI、最小演出。ただしCTAと配信予定の可読性を最優先にする。
...
- Audience language: ステルス/スパイ系 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-r&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Rogue Stealth のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
- Signature motion: Rogue Stealth の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Rogue Stealth」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
