# Streamer M: Metallic Chrome Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_m
- Existing path: `templates/streamer/m`
- Current theme name: Metallic Chrome
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Metallic Chrome」。features は Metallic, Liquid, Pro、主要色は #9ca3af, #111827, #000000。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 近未来/ファッション系
- Conversion goal: 最新配信、ビジュアルギャラリー、案件
- Core concept: 近未来/ファッション系 向けの公式サイト。既存の Metallic Chrome らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Visual hook: クローム反射、雑誌的Hero、UIは整理。ただしCTAと配信予定の可読性を最優先にする。
...
- Color: 既存の #9ca3af, #111827, #000000 を起点に、CTA色と装飾色を分ける。
...
- Audience language: 近未来/ファッション系 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-m&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Metallic Chrome のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Metallic Chrome」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
...
## 受け入れ条件
- テーマ名「Metallic Chrome」の印象が残っている。
