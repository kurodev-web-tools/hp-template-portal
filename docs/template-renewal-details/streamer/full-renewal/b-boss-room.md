# Streamer B: Boss Room Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_b
- Existing path: `templates/streamer/b`
- Current theme name: Boss Room
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Boss Room」。features は RPG Boss, Parallax, Dark Gold、主要色は #d4af37, #8a6d3b, #121212。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: RPGボス系VTuber
- Conversion goal: スケジュール、切り抜き、ファンコミュニティ
- Core concept: RPGボス系VTuber 向けの公式サイト。既存の Boss Room らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Visual hook: 玉座/ステータスUI、重厚な暗金、HPバー型セクション。ただしCTAと配信予定の可読性を最優先にする。
...
- Color: 既存の #d4af37, #8a6d3b, #121212 を起点に、CTA色と装飾色を分ける。
...
- Audience language: RPGボス系VTuber のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-b&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Boss Room のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Boss Room」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
