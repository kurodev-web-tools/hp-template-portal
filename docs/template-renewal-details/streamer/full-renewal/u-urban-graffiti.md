# Streamer U: Urban Graffiti Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_u
- Existing path: `templates/streamer/u`
- Current theme name: Urban Graffiti
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Urban Graffiti」。features は Graffiti, Street, Vivid、主要色は #fbbf24, #262626, #181818。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: ストリート/ラップ/カルチャー系
- Conversion goal: 配信/ライブ予定、SNS、コラボ相談
- Core concept: ストリート/ラップ/カルチャー系 向けの公式サイト。既存の Urban Graffiti らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Visual hook: 壁面グラフィティ、イベント告知、太いCTA。ただしCTAと配信予定の可読性を最優先にする。
...
- Color: 既存の #fbbf24, #262626, #181818 を起点に、CTA色と装飾色を分ける。
...
- Audience language: ストリート/ラップ/カルチャー系 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-u&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Urban Graffiti のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Urban Graffiti」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
