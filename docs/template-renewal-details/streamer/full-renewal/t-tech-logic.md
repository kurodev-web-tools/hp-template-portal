# Streamer T: Tech Logic Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_t
- Existing path: `templates/streamer/t`
- Current theme name: Tech Logic
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Tech Logic」。features は Tech, Logic, Circuit、主要色は #06b6d4, #1f2937, #111827。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 論理/開発/ガジェット系
- Conversion goal: 配信テーマ、技術相談、アーカイブ
- Core concept: 論理/開発/ガジェット系 向けの公式サイト。既存の Tech Logic らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Color: 既存の #06b6d4, #1f2937, #111827 を起点に、CTA色と装飾色を分ける。
...
- Audience language: 論理/開発/ガジェット系 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-t&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Tech Logic のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
- Signature motion: Tech Logic の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Tech Logic」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
