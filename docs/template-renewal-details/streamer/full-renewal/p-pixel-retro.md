# Streamer P: Pixel Retro Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_p
- Existing path: `templates/streamer/p`
- Current theme name: Pixel Retro
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Pixel Retro」。features は 8-bit, Pixel Art, Retro、主要色は #22c55e, #795548, #000000。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: レトロゲーム配信者
- Conversion goal: 配信予定、ゲームリスト、Discord
- Core concept: レトロゲーム配信者 向けの公式サイト。既存の Pixel Retro らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Visual hook: ドットUI、セーブスロット、カセット風カード。ただしCTAと配信予定の可読性を最優先にする。
...
- Color: 既存の #22c55e, #795548, #000000 を起点に、CTA色と装飾色を分ける。
...
- Audience language: レトロゲーム配信者 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-p&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Pixel Retro のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Pixel Retro」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
