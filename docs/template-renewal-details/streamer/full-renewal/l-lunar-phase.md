# Streamer L: Lunar Phase Renewal Detail

## 基本情報
- Category: Streamer
- Template ID: st_l
- Existing path: `templates/streamer/l`
- Current theme name: Lunar Phase
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Lunar Phase」。features は Lunar, Fantasy, Calm、主要色は #fef3c7, #4c1d95, #1e1b4b。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 月/占い/夜系配信者
- Conversion goal: 配信予定、占い/相談、SNS
- Core concept: 月/占い/夜系配信者 向けの公式サイト。既存の Lunar Phase らしさを保ちつつ、配信予定、SNS、アーカイブ、案件相談へつなぐ。
...
- Visual hook: 月齢UI、静かなグラデーション、夜の予定表。ただしCTAと配信予定の可読性を最優先にする。
...
- Audience language: 月/占い/夜系配信者 のファンが読む言葉と、企業担当者が読む案件相談の言葉を分ける。
...
- Portal CTA: `plans.html?template=streamer-l&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 世界観演出は Lunar Phase のHeroと1セクションに限定し、配信予定やSNS導線の可読性を優先する。
...
- Signature motion: Lunar Phase の世界観をHeroに1つだけ置く。例: status pulse、schedule reveal、UI scan、背景レイヤーの緩い移動。
...
## モック画像方針
必須。first viewport + schedule / clips teaser を 1 枚作る。既存テーマ名「Lunar Phase」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
