# Wide Pan Implementation Plan

**Goal:** Wide Pan をモック準拠の高品質な 旅/風景/雑談配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`ultra-wide cinematic stream wall` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/w/assets/`.
...
- Modify only `public/templates/streamer/w/**`.
- Rebuild or strengthen the landing page structure around the `Wide Pan` mock: top minimal frame labels, center panoramic hero, bottom horizontal timeline for Live/Schedule/Archive, far-right contact slate.
...
- Keep all assets template-local under `public/templates/streamer/w/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-w&plan=standard`.
...
- Standard motion: `wide pan reveal`, `timeline slide`, `night light pulse`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/w-wide-pan/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/polish/w-wide-pan-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/polish/w-wide-pan.md`
- Modify: `public/templates/streamer/w/index.html`
- Create/modify: `public/templates/streamer/w/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/w/assets/js/script.js`
- Create if needed: `public/templates/streamer/w/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `ultra-wide cinematic stream wall` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-w&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/w/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `WideNav`, `PanHero`, `CinemaSchedule`, `SceneFollow`, `TourContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 旅/風景/雑談配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `wide pan reveal` with CSS first.
- [ ] Add `timeline slide` with CSS first.
- [ ] Add `night light pulse` with CSS first.
...
- First viewport should clearly communicate `Wide Pan` and should follow this placement: top minimal frame labels, center panoramic hero, bottom horizontal timeline for Live/Schedule/Archive, far-right contact slate.
...
- 旅/風景/雑談配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/w/**` に閉じている。
