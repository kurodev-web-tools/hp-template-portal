# Metallic Chrome Implementation Plan

**Goal:** Metallic Chrome をモック準拠の高品質な 近未来/ファッション系 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`chrome showroom command display` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/m/assets/`.
...
- Modify only `public/templates/streamer/m/**`.
- Rebuild or strengthen the landing page structure around the `Metallic Chrome` mock: center chrome hero pedestal, left spec/navigation strip, right live demo/schedule display, bottom polished contact dock.
...
- Keep all assets template-local under `public/templates/streamer/m/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-m&plan=standard`.
...
- Standard motion: `chrome sheen`, `spec panel reveal`, `live demo glow`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/m-metallic-chrome/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/m-metallic-chrome-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/m-metallic-chrome.md`
- Modify: `public/templates/streamer/m/index.html`
- Create/modify: `public/templates/streamer/m/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/m/assets/js/script.js`
- Create if needed: `public/templates/streamer/m/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `chrome showroom command display` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-m&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/m/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `ChromeNav`, `MetallicHero`, `SpecSchedule`, `ShowroomFollow`, `ChromeContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 近未来/ファッション系.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `chrome sheen` with CSS first.
- [ ] Add `spec panel reveal` with CSS first.
- [ ] Add `live demo glow` with CSS first.
...
- First viewport should clearly communicate `Metallic Chrome` and should follow this placement: center chrome hero pedestal, left spec/navigation strip, right live demo/schedule display, bottom polished contact dock.
...
- 近未来/ファッション系 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/m/**` に閉じている。
