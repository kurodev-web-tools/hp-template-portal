# Orbit Space Implementation Plan

**Goal:** Orbit Space をモック準拠の高品質な 宇宙/科学系配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`orbital mission control` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/o/assets/`.
...
- Modify only `public/templates/streamer/o/**`.
- Rebuild or strengthen the landing page structure around the `Orbit Space` mock: center planet/avatar orbit, circular nav, right mission schedule, left telemetry/live status, bottom contact as ground control channel.
...
- Keep all assets template-local under `public/templates/streamer/o/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-o&plan=standard`.
...
- Standard motion: `orbital line sweep`, `mission panel reveal`, `telemetry pulse`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/o-orbit-space/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/o-orbit-space-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/o-orbit-space.md`
- Modify: `public/templates/streamer/o/index.html`
- Create/modify: `public/templates/streamer/o/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/o/assets/js/script.js`
- Create if needed: `public/templates/streamer/o/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `orbital mission control` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-o&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/o/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `MissionNav`, `OrbitHero`, `TelemetrySchedule`, `OrbitFollow`, `GroundContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 宇宙/科学系配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `orbital line sweep` with CSS first.
- [ ] Add `mission panel reveal` with CSS first.
- [ ] Add `telemetry pulse` with CSS first.
...
- First viewport should clearly communicate `Orbit Space` and should follow this placement: center planet/avatar orbit, circular nav, right mission schedule, left telemetry/live status, bottom contact as ground control channel.
...
- 宇宙/科学系配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/o/**` に閉じている。
