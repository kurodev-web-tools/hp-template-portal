# E-Sports Pro Implementation Plan

**Goal:** E-Sports Pro をモック準拠の高品質な 競技系配信者/チーム 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`e-sports broadcast match room` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/e/assets/`.
...
- Modify only `public/templates/streamer/e/**`.
- Rebuild or strengthen the landing page structure around the `E-Sports Pro` mock: top scoreboard header, center match/map hero, left roster/platform panel, right Next Match bracket, bottom sponsor/contact strip.
...
- Keep all assets template-local under `public/templates/streamer/e/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-e&plan=standard`.
...
- Standard motion: `score ticker reveal`, `match panel reveal`, `live indicator pulse`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/e-e-sports-pro/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/partial-renewal/e-e-sports-pro-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/partial-renewal/e-e-sports-pro.md`
- Modify: `public/templates/streamer/e/index.html`
- Modify: `public/templates/streamer/e/contact.html`
- Create/modify: `public/templates/streamer/e/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/e/assets/js/script.js`
- Create if needed: `public/templates/streamer/e/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `e-sports broadcast match room` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-e&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/e/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `RosterRail`, `MatchHero`, `TournamentSchedule`, `TeamFollow`, `SponsorContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 競技系配信者/チーム.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `score ticker reveal` with CSS first.
- [ ] Add `match panel reveal` with CSS first.
- [ ] Add `live indicator pulse` with CSS first.
...
- First viewport should clearly communicate `E-Sports Pro` and should follow this placement: top scoreboard header, center match/map hero, left roster/platform panel, right Next Match bracket, bottom sponsor/contact strip.
...
- 競技系配信者/チーム 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/e/**` に閉じている。
