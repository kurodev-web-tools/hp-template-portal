# Crystal Prism Implementation Plan

**Goal:** Crystal Prism をモック準拠の高品質な 透明感/癒し/歌枠配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`airy floating prism hub` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/c/assets/`.
...
- Modify only `public/templates/streamer/c/**`.
- Rebuild or strengthen the landing page structure around the `Crystal Prism` mock: hero identity upper-left to center, Next Stream as a floating prism near center-right, Song Archive as a waveform panel near lower-left or lower-center, Follow as small icon crystals, Contact as a quiet lower-right glass chip.
...
- Keep all assets template-local under `public/templates/streamer/c/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-c&plan=standard`.
...
- Standard motion: `prism shimmer`, `floating panel reveal`, `soft live glow`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/c-crystal-prism/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/c-crystal-prism-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/c-crystal-prism.md`
- Modify: `public/templates/streamer/c/index.html`
- Create/modify: `public/templates/streamer/c/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/c/assets/js/script.js`
- Create if needed: `public/templates/streamer/c/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `airy floating prism hub` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-c&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/c/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `PrismNav`, `CrystalHero`, `FloatingSchedule`, `SoftFollow`, `PrismContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 透明感/癒し/歌枠配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `prism shimmer` with CSS first.
- [ ] Add `floating panel reveal` with CSS first.
- [ ] Add `soft live glow` with CSS first.
...
- First viewport should clearly communicate `Crystal Prism` and should follow this placement: hero identity upper-left to center, Next Stream as a floating prism near center-right, Song Archive as a waveform panel near lower-left or lower-center, Follow as small icon crystals, Contact as a quiet lower-right glass chip.
...
- 透明感/癒し/歌枠配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/c/**` に閉じている。
