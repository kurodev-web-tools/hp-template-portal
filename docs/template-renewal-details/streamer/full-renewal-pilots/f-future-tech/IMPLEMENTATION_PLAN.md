# Future Tech Implementation Plan

**Goal:** Future Tech をモック準拠の高品質な 近未来/AI系配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`future lab command deck` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/f/assets/`.
...
- Modify only `public/templates/streamer/f/**`.
- Rebuild or strengthen the landing page structure around the `Future Tech` mock: top minimal lab nav, center prototype/AI core hero, left module nav, right Live demo/Schedule panels, bottom inquiry dock.
...
- Keep all assets template-local under `public/templates/streamer/f/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-f&plan=standard`.
...
- Standard motion: `lab scan`, `module reveal`, `hologram pulse`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/f-future-tech/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/f-future-tech-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/f-future-tech.md`
- Modify: `public/templates/streamer/f/index.html`
- Create/modify: `public/templates/streamer/f/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/f/assets/js/script.js`
- Create if needed: `public/templates/streamer/f/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `future lab command deck` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-f&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/f/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `LabNav`, `FutureHero`, `ModuleSchedule`, `TechFollow`, `ProtocolContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 近未来/AI系配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `lab scan` with CSS first.
- [ ] Add `module reveal` with CSS first.
- [ ] Add `hologram pulse` with CSS first.
...
- First viewport should clearly communicate `Future Tech` and should follow this placement: top minimal lab nav, center prototype/AI core hero, left module nav, right Live demo/Schedule panels, bottom inquiry dock.
...
- 近未来/AI系配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/f/**` に閉じている。
