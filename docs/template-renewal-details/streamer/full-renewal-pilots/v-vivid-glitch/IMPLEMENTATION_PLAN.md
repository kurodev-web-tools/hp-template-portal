# Vivid Glitch Implementation Plan

**Goal:** Vivid Glitch をモック準拠の高品質な 短尺/高速編集系 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`vivid glitch sticker stack` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/v/assets/`.
...
- Modify only `public/templates/streamer/v/**`.
- Rebuild or strengthen the landing page structure around the `Vivid Glitch` mock: offset sticker nav, center oversized hero sticker, floating live/clip windows, diagonal schedule strip, separated contact sticker.
...
- Keep all assets template-local under `public/templates/streamer/v/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-v&plan=standard`.
...
- Standard motion: `sticker slam`, `glitch cut`, `clip panel reveal`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/v-vivid-glitch/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/v-vivid-glitch-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/v-vivid-glitch.md`
- Modify: `public/templates/streamer/v/index.html`
- Create/modify: `public/templates/streamer/v/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/v/assets/js/script.js`
- Create if needed: `public/templates/streamer/v/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `vivid glitch sticker stack` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-v&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/v/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `StickerNav`, `VividHero`, `ClipSchedule`, `PopFollow`, `CollabContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 短尺/高速編集系.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `sticker slam` with CSS first.
- [ ] Add `glitch cut` with CSS first.
...
- First viewport should clearly communicate `Vivid Glitch` and should follow this placement: offset sticker nav, center oversized hero sticker, floating live/clip windows, diagonal schedule strip, separated contact sticker.
...
- 短尺/高速編集系 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/v/**` に閉じている。
