# Pixel Retro Implementation Plan

**Goal:** Pixel Retro をモック準拠の高品質な レトロゲーム配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`retro game save-select screen` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/p/assets/`.
...
- Modify only `public/templates/streamer/p/**`.
- Rebuild or strengthen the landing page structure around the `Pixel Retro` mock: top pixel title bar, center character/save-slot hero, left menu commands, right next stage/schedule, bottom cartridge Contact.
...
- Keep all assets template-local under `public/templates/streamer/p/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-p&plan=standard`.
...
- Standard motion: `pixel boot`, `save slot select`, `steps hover`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/p-pixel-retro/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/p-pixel-retro-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/p-pixel-retro.md`
- Modify: `public/templates/streamer/p/index.html`
- Create/modify: `public/templates/streamer/p/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/p/assets/js/script.js`
- Create if needed: `public/templates/streamer/p/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `retro game save-select screen` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-p&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/p/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `SaveMenuNav`, `PixelHero`, `SlotSchedule`, `PartyFollow`, `ArcadeContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for レトロゲーム配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `pixel boot` with CSS first.
- [ ] Add `save slot select` with CSS first.
- [ ] Add `steps hover` with CSS first.
...
- First viewport should clearly communicate `Pixel Retro` and should follow this placement: top pixel title bar, center character/save-slot hero, left menu commands, right next stage/schedule, bottom cartridge Contact.
...
- レトロゲーム配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/p/**` に閉じている。
