# Digital Ghost Implementation Plan

**Goal:** Digital Ghost をモック準拠の高品質な 匿名/ミステリアス系配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`anonymous terminal breach console` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/d/assets/`.
...
- Modify only `public/templates/streamer/d/**`.
- Rebuild or strengthen the landing page structure around the `Digital Ghost` mock: top command prompt/nav, left access log timeline, center ghost identity glitch silhouette, right encrypted Live/Schedule terminal, bottom secure channel for Follow/Contact.
...
- Keep all assets template-local under `public/templates/streamer/d/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-d&plan=standard`.
...
- Standard motion: `terminal boot`, `access scan`, `ghost line flicker`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/d-digital-ghost/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/d-digital-ghost-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/d-digital-ghost.md`
- Modify: `public/templates/streamer/d/index.html`
- Create/modify: `public/templates/streamer/d/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/d/assets/js/script.js`
- Create if needed: `public/templates/streamer/d/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `anonymous terminal breach console` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-d&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/d/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `GhostTerminalNav`, `AnonymousHero`, `SignalSchedule`, `EncryptedFollow`, `SecureContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 匿名/ミステリアス系配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `terminal boot` with CSS first.
- [ ] Add `access scan` with CSS first.
- [ ] Add `ghost line flicker` with CSS first.
...
- First viewport should clearly communicate `Digital Ghost` and should follow this placement: top command prompt/nav, left access log timeline, center ghost identity glitch silhouette, right encrypted Live/Schedule terminal, bottom secure channel for Follow/Contact.
...
- 匿名/ミステリアス系配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/d/**` に閉じている。
