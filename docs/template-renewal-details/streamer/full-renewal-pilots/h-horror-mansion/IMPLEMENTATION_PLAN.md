# Horror Mansion Implementation Plan

**Goal:** Horror Mansion をモック準拠の高品質な ホラー/怪談系VTuber 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`haunted mansion invitation map` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/h/assets/`.
...
- Modify only `public/templates/streamer/h/**`.
- Rebuild or strengthen the landing page structure around the `Horror Mansion` mock: left mansion floor map/nav, center hallway/portrait hero, right candlelit Next Stream invitation, bottom sealed Contact letter.
...
- Keep all assets template-local under `public/templates/streamer/h/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-h&plan=standard`.
...
- Standard motion: `candle flicker`, `invitation card reveal`, `mansion map focus`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/h-horror-mansion/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/h-horror-mansion-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/h-horror-mansion.md`
- Modify: `public/templates/streamer/h/index.html`
- Create/modify: `public/templates/streamer/h/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/h/assets/js/script.js`
- Create if needed: `public/templates/streamer/h/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `haunted mansion invitation map` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-h&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/h/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `MansionNav`, `InvitationHero`, `HauntSchedule`, `LanternFollow`, `SealedContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for ホラー/怪談系VTuber.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `candle flicker` with CSS first.
- [ ] Add `invitation card reveal` with CSS first.
- [ ] Add `mansion map focus` with CSS first.
...
- First viewport should clearly communicate `Horror Mansion` and should follow this placement: left mansion floor map/nav, center hallway/portrait hero, right candlelit Next Stream invitation, bottom sealed Contact letter.
...
- ホラー/怪談系VTuber 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/h/**` に閉じている。
