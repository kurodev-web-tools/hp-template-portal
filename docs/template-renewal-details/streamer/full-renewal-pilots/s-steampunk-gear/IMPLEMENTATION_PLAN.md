# Steampunk Gear Implementation Plan

**Goal:** Steampunk Gear をモック準拠の高品質な スチームパンク系 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`steampunk workshop control bench` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/s/assets/`.
...
- Modify only `public/templates/streamer/s/**`.
- Rebuild or strengthen the landing page structure around the `Steampunk Gear` mock: left gear nav, center engine/character hero, right pressure-gauge schedule, bottom brass contact work order.
...
- Keep all assets template-local under `public/templates/streamer/s/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-s&plan=standard`.
...
- Standard motion: `gear tick`, `gauge fill`, `workbench panel reveal`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/s-steampunk-gear/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/polish/s-steampunk-gear-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/polish/s-steampunk-gear.md`
- Modify: `public/templates/streamer/s/index.html`
- Modify: `public/templates/streamer/s/contact.html`
- Create/modify: `public/templates/streamer/s/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/s/assets/js/script.js`
- Create if needed: `public/templates/streamer/s/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `steampunk workshop control bench` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-s&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/s/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `WorkshopNav`, `SteampunkHero`, `GearSchedule`, `BrassFollow`, `WorkOrderContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for スチームパンク系.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `gear tick` with CSS first.
- [ ] Add `gauge fill` with CSS first.
- [ ] Add `workbench panel reveal` with CSS first.
...
- First viewport should clearly communicate `Steampunk Gear` and should follow this placement: left gear nav, center engine/character hero, right pressure-gauge schedule, bottom brass contact work order.
...
- スチームパンク系 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/s/**` に閉じている。
