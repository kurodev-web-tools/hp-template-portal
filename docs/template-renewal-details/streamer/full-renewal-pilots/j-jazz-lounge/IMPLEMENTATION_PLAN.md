# Jazz Lounge Implementation Plan

**Goal:** Jazz Lounge をモック準拠の高品質な 音楽/夜配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`night jazz lounge reservation menu` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/j/assets/`.
...
- Modify only `public/templates/streamer/j/**`.
- Rebuild or strengthen the landing page structure around the `Jazz Lounge` mock: left vertical lounge menu/nav, center stage or record-cover hero, right Tonight set list/Next Stream, bottom reservation-style Contact.
...
- Keep all assets template-local under `public/templates/streamer/j/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-j&plan=standard`.
...
- Standard motion: `curtain fade`, `microphone glow`, `reservation panel reveal`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/j-jazz-lounge/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/j-jazz-lounge-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/j-jazz-lounge.md`
- Modify: `public/templates/streamer/j/index.html`
- Create/modify: `public/templates/streamer/j/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/j/assets/js/script.js`
- Create if needed: `public/templates/streamer/j/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `night jazz lounge reservation menu` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-j&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/j/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `LoungeNav`, `JazzHero`, `TonightSchedule`, `VelvetFollow`, `ReservationContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 音楽/夜配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `curtain fade` with CSS first.
- [ ] Add `microphone glow` with CSS first.
- [ ] Add `reservation panel reveal` with CSS first.
...
- First viewport should clearly communicate `Jazz Lounge` and should follow this placement: left vertical lounge menu/nav, center stage or record-cover hero, right Tonight set list/Next Stream, bottom reservation-style Contact.
...
- 音楽/夜配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/j/**` に閉じている。
