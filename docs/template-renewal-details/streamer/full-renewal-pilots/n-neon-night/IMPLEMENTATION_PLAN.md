# Neon Night Implementation Plan

**Goal:** Neon Night をモック準拠の高品質な 夜遊び/クラブ配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`neon nightlife sign street` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/n/assets/`.
...
- Modify only `public/templates/streamer/n/**`.
- Rebuild or strengthen the landing page structure around the `Neon Night` mock: top club sign/nav, center street hero, right event board schedule, bottom flyer stickers for Follow/Contact.
...
- Keep all assets template-local under `public/templates/streamer/n/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-n&plan=standard`.
...
- Standard motion: `neon sign flicker`, `marquee reveal`, `schedule pulse`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/n-neon-night/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/n-neon-night-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/n-neon-night.md`
- Modify: `public/templates/streamer/n/index.html`
- Create/modify: `public/templates/streamer/n/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/n/assets/js/script.js`
- Create if needed: `public/templates/streamer/n/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `neon nightlife sign street` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-n&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/n/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `NeonSignNav`, `NightHero`, `ClubSchedule`, `BarFollow`, `BookingContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 夜遊び/クラブ配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `neon sign flicker` with CSS first.
- [ ] Add `marquee reveal` with CSS first.
- [ ] Add `schedule pulse` with CSS first.
...
- First viewport should clearly communicate `Neon Night` and should follow this placement: top club sign/nav, center street hero, right event board schedule, bottom flyer stickers for Follow/Contact.
...
- 夜遊び/クラブ配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/n/**` に閉じている。
