# Tech Logic Implementation Plan

**Goal:** Tech Logic をモック準拠の高品質な 論理/開発/ガジェット系 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`logic circuit operations board` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/t/assets/`.
...
- Modify only `public/templates/streamer/t/**`.
- Rebuild or strengthen the landing page structure around the `Tech Logic` mock: top compact command bar, center node graph hero, left build queue, right live/schedule verification panel, bottom API-like contact.
...
- Keep all assets template-local under `public/templates/streamer/t/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-t&plan=standard`.
...
- Standard motion: `circuit trace`, `node reveal`, `logic panel pulse`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/t-tech-logic/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/t-tech-logic-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/t-tech-logic.md`
- Modify: `public/templates/streamer/t/index.html`
- Create/modify: `public/templates/streamer/t/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/t/assets/js/script.js`
- Create if needed: `public/templates/streamer/t/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `logic circuit operations board` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-t&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/t/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `LogicNav`, `TechHero`, `FlowSchedule`, `SystemFollow`, `ApiContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 論理/開発/ガジェット系.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `circuit trace` with CSS first.
- [ ] Add `node reveal` with CSS first.
- [ ] Add `logic panel pulse` with CSS first.
...
- First viewport should clearly communicate `Tech Logic` and should follow this placement: top compact command bar, center node graph hero, left build queue, right live/schedule verification panel, bottom API-like contact.
...
- 論理/開発/ガジェット系 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/t/**` に閉じている。
