# Abyss Neon Implementation Plan

**Goal:** Abyss Neon をモック準拠の高品質な 深海サイバー配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`deep-sea neon sonar cockpit` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/a/assets/`.
...
- Modify only `public/templates/streamer/a/**`.
- Rebuild or strengthen the landing page structure around the `Abyss Neon` mock: left vertical depth meter/nav, center submerged hero identity, right sonar Live/Next Stream beacon, bottom dock for Follow and Contact.
...
- Keep all assets template-local under `public/templates/streamer/a/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-a&plan=standard`.
...
- Standard motion: `sonar sweep`, `depth panel reveal`, `live beacon pulse`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/a-abyss-neon/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/polish/a-abyss-neon-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/polish/a-abyss-neon.md`
- Modify: `public/templates/streamer/a/index.html`
- Create/modify: `public/templates/streamer/a/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/a/assets/js/script.js`
- Create if needed: `public/templates/streamer/a/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `deep-sea neon sonar cockpit` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-a&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/a/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `DepthNav`, `AbyssHero`, `SonarSchedule`, `SignalFollow`, `RescueContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for 深海サイバー配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `sonar sweep` with CSS first.
- [ ] Add `depth panel reveal` with CSS first.
- [ ] Add `live beacon pulse` with CSS first.
...
- First viewport should clearly communicate `Abyss Neon` and should follow this placement: left vertical depth meter/nav, center submerged hero identity, right sonar Live/Next Stream beacon, bottom dock for Follow and Contact.
...
- 深海サイバー配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/a/**` に閉じている。
