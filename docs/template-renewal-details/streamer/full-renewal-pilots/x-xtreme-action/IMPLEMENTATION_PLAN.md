# Xtreme Action Implementation Plan

**Goal:** Xtreme Action をモック準拠の高品質な スポーツ/アクション配信者 向け配信者公式サイトへリニューアルする。

**Architecture:** Static-first。`action sports camera overlay` を first viewport の基準にし、Phase 1 で HTML/CSS 構造、Phase 2 で素材と visual fidelity、Phase 3 で軽量 signature motion を追加する。PC はテーマ世界観を優先し、SP は CTA と schedule の到達性を優先する。
...
- Assets: SVG/WebP under `public/templates/streamer/x/assets/`.
...
- Modify only `public/templates/streamer/x/**`.
- Rebuild or strengthen the landing page structure around the `Xtreme Action` mock: diagonal top timer/nav, center action hero, left speed/status meter, right event schedule, bottom sponsor/contact rail.
...
- Keep all assets template-local under `public/templates/streamer/x/assets/`.
- Keep portal CTA links pointing to `plans.html?template=streamer-x&plan=standard`.
...
- Standard motion: `diagonal wipe`, `speed gauge fill`, `action card snap`.
...
- Reference: `docs/template-renewal-details/streamer/full-renewal-pilots/x-xtreme-action/DESIGN.md`
- Reference: `docs/template-renewal-details/streamer/mockup-image/full-renewal/x-xtreme-action-layout-v2.png`
- Reference: `docs/template-renewal-details/streamer/full-renewal/x-xtreme-action.md`
- Modify: `public/templates/streamer/x/index.html`
- Create/modify: `public/templates/streamer/x/assets/css/style.css`
- Create/modify if needed: `public/templates/streamer/x/assets/js/script.js`
- Create if needed: `public/templates/streamer/x/assets/img/`

## Phase 1: Static Structure

- [ ] Create the PC layout based on the mock's `action sports camera overlay` signature.
- [ ] Add semantic sections: `hero`, `profile`, `schedule`, `archive`, `community`, `contact`.
- [ ] Add SP order independent from PC visual order.
- [ ] Add CTA links to `plans.html?template=streamer-x&plan=standard`.
- [ ] Keep all links and asset paths local to `public/templates/streamer/x/**` where possible.

## Phase 2: Visual Fidelity

- [ ] Define CSS variables from `DESIGN.md`.
- [ ] Build template-local component classes for `ActionNav`, `XtremeHero`, `RaceSchedule`, `SponsorFollow`, `ActionContact`.
- [ ] Add SVG/WebP assets only where CSS cannot reproduce the mock quality.
- [ ] Replace demo copy with believable streamer official-site copy for スポーツ/アクション配信者.
- [ ] Preserve the mock's card shapes, information placement, and CTA hierarchy.

## Phase 3: Signature Motion

- [ ] Add `diagonal wipe` with CSS first.
- [ ] Add `speed gauge fill` with CSS first.
- [ ] Add `action card snap` with CSS first.
...
- First viewport should clearly communicate `Xtreme Action` and should follow this placement: diagonal top timer/nav, center action hero, left speed/status meter, right event schedule, bottom sponsor/contact rail.
...
- スポーツ/アクション配信者 向けの配信者公式サイトとして、企業 HP 風ではなくテーマ固有の世界観がある。
- Live / Schedule / Follow / Community / Contact の導線が迷わない。
- 標準演出を止めてもテンプレートとして成立する。
- 変更範囲が `public/templates/streamer/x/**` に閉じている。
