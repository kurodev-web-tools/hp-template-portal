...
Urban Graffiti は ストリート/ラップ/カルチャー系 向けの配信者公式サイト。Conversion goal は 配信/ライブ予定、SNS、コラボ相談。モックの first viewport を基準に、テーマ固有のレイアウト、装飾、カード形状、CTA 位置を販売用テンプレート品質で再現する。
...
標準演出は `poster paste`、`sticker pop`、`waveform pulse`。実装は CSS transitions/animations と vanilla JavaScript を優先する。GSAP は CSS だけだと brittle になる短い Phase 3 signature motion に限り許可する。
