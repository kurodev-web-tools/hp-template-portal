# Portfolio Y: Yarn Renewal Detail

## 基本情報
- Category: Portfolio
- Template ID: pf_y
- Existing path: `templates/portfolio/y`
- Current theme name: Yarn
- Renewal decision: Polish
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Yarn」。features は Yarn, Dashed, Handcraft、主要色は #fff1f2, #e11d48, #fecdd3。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: ハンドクラフト作家
- Conversion goal: 作品閲覧、問い合わせ、案件相談、プロフィール理解
- Core concept: ハンドクラフト作家 の作品サイト。見た目だけでなく、誰に何を依頼できるかを伝える。
...
- Visual hook: 販売、展示、依頼の分岐。作品を隠す装飾や過剰な文字被せは避ける。
...
- Visual direction: 販売、展示、依頼の分岐
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #fff1f2, #e11d48, #fecdd3 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #fff1f2, #e11d48, #fecdd3 は作品の邪魔をしない背景/アクセントに限定する。
...
- Audience language: ハンドクラフト作家 が「この人に何を頼めるか」を判断できる言葉にする。
...
- Portal CTA: `plans.html?template=portfolio-y&plan=standard` へ接続する想定。
