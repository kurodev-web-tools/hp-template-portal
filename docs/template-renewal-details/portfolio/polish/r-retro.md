# Portfolio R: Retro Renewal Detail

## 基本情報
- Category: Portfolio
- Template ID: pf_r
- Existing path: `templates/portfolio/r`
- Current theme name: Retro
- Renewal decision: Polish
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Retro」。features は Retro, Glitch, 8-bit、主要色は #172554, #facc15, #ef4444。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: インディーゲーム開発者
- Conversion goal: 作品閲覧、問い合わせ、案件相談、プロフィール理解
- Core concept: インディーゲーム開発者 の作品サイト。見た目だけでなく、誰に何を依頼できるかを伝える。
...
- Visual hook: 作品詳細と体験版/連絡導線。作品を隠す装飾や過剰な文字被せは避ける。
...
- Visual direction: 作品詳細と体験版/連絡導線
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #172554, #facc15, #ef4444 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #172554, #facc15, #ef4444 は作品の邪魔をしない背景/アクセントに限定する。
...
- Audience language: インディーゲーム開発者 が「この人に何を頼めるか」を判断できる言葉にする。
...
- Portal CTA: `plans.html?template=portfolio-r&plan=standard` へ接続する想定。
