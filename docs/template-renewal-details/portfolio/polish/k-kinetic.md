# Portfolio K: Kinetic Renewal Detail

## 基本情報
- Category: Portfolio
- Template ID: pf_k
- Existing path: `templates/portfolio/k`
- Current theme name: Kinetic
- Renewal decision: Polish
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Kinetic」。features は Kinetic, Physics, Motion、主要色は #ef4444, #171717, #ffffff。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: モーションデザイナー
- Conversion goal: 作品閲覧、問い合わせ、案件相談、プロフィール理解
- Core concept: モーションデザイナー の作品サイト。見た目だけでなく、誰に何を依頼できるかを伝える。
...
- Visual hook: 動きは作品理解を助ける範囲にする。作品を隠す装飾や過剰な文字被せは避ける。
...
- Visual direction: 動きは作品理解を助ける範囲にする
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #ef4444, #171717, #ffffff を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #ef4444, #171717, #ffffff は作品の邪魔をしない背景/アクセントに限定する。
...
- Audience language: モーションデザイナー が「この人に何を頼めるか」を判断できる言葉にする。
...
- Portal CTA: `plans.html?template=portfolio-k&plan=standard` へ接続する想定。
