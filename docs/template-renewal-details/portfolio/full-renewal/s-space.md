# Portfolio S: Space Renewal Detail

## 基本情報
- Category: Portfolio
- Template ID: pf_s
- Existing path: `templates/portfolio/s`
- Current theme name: Space
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Space」。features は Space, Orbit, Aerospace、主要色は #0f172a, #e2e8f0, #3b82f6。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 3D/宇宙/空間クリエイター
- Conversion goal: 作品閲覧、問い合わせ、案件相談、プロフィール理解
- Core concept: 3D/宇宙/空間クリエイター の作品サイト。見た目だけでなく、誰に何を依頼できるかを伝える。
...
- Visual hook: 軌道UIを作品閲覧に接続して全面再構成。作品を隠す装飾や過剰な文字被せは避ける。
...
- Visual direction: 軌道UIを作品閲覧に接続して全面再構成
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #0f172a, #e2e8f0, #3b82f6 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #0f172a, #e2e8f0, #3b82f6 は作品の邪魔をしない背景/アクセントに限定する。
...
- Audience language: 3D/宇宙/空間クリエイター が「この人に何を頼めるか」を判断できる言葉にする。
...
- Portal CTA: `plans.html?template=portfolio-s&plan=standard` へ接続する想定。
- Demo CTA: fixed bottom CTA または Hero CTA から「このテンプレートで制作相談」へ戻す。

## Motion / Effect
- Full renewal: 作品の見せ方を強める遷移に限定し、Space の装飾が作品を上書きしないようにする。
...
## モック画像方針
必須。first viewport + selected work detail teaser を 1 枚作る。既存テーマ名「Space」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
