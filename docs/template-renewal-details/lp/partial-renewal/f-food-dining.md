# LP F: Food & Dining Renewal Detail

## 基本情報
- Category: LP
- Template ID: lp_f
- Existing path: `templates/lp/f`
- Current theme name: Food & Dining
- Renewal decision: Partial Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Food & Dining」。features は Food, Restaurant, Delicious、主要色は #ea580c, #fff7ed, #292524。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: Food & Dining の offer を探している見込み客
- Conversion goal: 飲食予約
- Core concept: 飲食予約 に絞ったCV型LP。1ページ1目的で、訴求、証拠、価格/条件、CTAを迷わせない。
...
- Visual hook: メニュー、席予約、アクセス。演出よりoffer理解とCTA視認性を優先する。
- Primary actions: Conversion goal (飲食予約) に直結するCTAを1つ主役にする。
...
- Visual direction: メニュー、席予約、アクセス
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #ea580c, #fff7ed, #292524 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #ea580c, #fff7ed, #292524 を背景、信頼材料、CTAで役割分担する。CTA色を複数に分散させない。
...
- Audience language: Food & Dining の offer を探している見込み客 が今申し込む理由を理解できる言葉にする。
- Required concrete details: Conversion goal (飲食予約) の条件、ベネフィット、証拠、FAQ、送信後の流れ。
- Avoid: 「今すぐ」「簡単」だけで、内容や条件が薄い状態。

## CTA / 導線
- Primary CTA: 申し込む / 予約する / 無料体験する / 資料請求
- Secondary CTA: FAQ、価格、レビュー、比較表
- Portal CTA: `plans.html?template=lp-f&plan=standard` へ接続する想定。
