# LP Y: Youth Culture Renewal Detail

## 基本情報
- Category: LP
- Template ID: lp_y
- Existing path: `templates/lp/y`
- Current theme name: Youth Culture
- Renewal decision: Polish
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Youth Culture」。features は Youth, Trend, Pop、主要色は #fbbf24, #fef3c7, #000000。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: Youth Culture の offer を探している見込み客
- Conversion goal: イベント/商品購入
- Core concept: イベント/商品購入 に絞ったCV型LP。1ページ1目的で、訴求、証拠、価格/条件、CTAを迷わせない。
- Priority: 既存の完成度を活かし、受注導線と文言を磨く。

## 詳細分類
- Strategic renewal depth: Low。文言、価格/条件、CTA視認性を中心に磨く。
...
- Visual hook: 若年層向けCTAとSNS導線。演出よりoffer理解とCTA視認性を優先する。
- Primary actions: Conversion goal (イベント/商品購入) に直結するCTAを1つ主役にする。
- SP order: offer、CTA、安心材料、価格/条件、フォームの順にする。

## Section-by-Section Plan
- Hero Offer (既存維持 + 改善): 申込理由を最短で作る。入れる内容: offer、対象者、価格/無料条件、CTA。注意: キャッチコピーだけで内容を隠さない。
- Problem / Promise (既存維持 + 改善): 課題と解決後を接続する。入れる内容: 悩み、Before/After、約束。注意: 煽りすぎない。
- Benefits (既存維持 + 改善): 選ぶ理由を比較可能にする。入れる内容: 3-5 benefit、成果、対象条件。注意: 抽象的な利点だけにしない。
- Proof / Pricing (既存維持 + 改善): 不安を下げる。入れる内容: レビュー、実績、比較、価格/プラン。注意: 価格や条件を隠しすぎない。
- FAQ / Form (既存維持 + 改善): 行動直前の不安を潰す。入れる内容: FAQ、入力項目、送信後の流れ。注意: フォームを長くしすぎない。

## デザイン / レイアウト方針
- Layout base: 1ページ1目的を徹底し、CTAとフォーム摩擦を最小化する。
- Visual direction: 若年層向けCTAとSNS導線
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #fbbf24, #fef3c7, #000000 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #fbbf24, #fef3c7, #000000 を背景、信頼材料、CTAで役割分担する。CTA色を複数に分散させない。
...
- Audience language: Youth Culture の offer を探している見込み客 が今申し込む理由を理解できる言葉にする。
- Required concrete details: Conversion goal (イベント/商品購入) の条件、ベネフィット、証拠、FAQ、送信後の流れ。
- Avoid: 「今すぐ」「簡単」だけで、内容や条件が薄い状態。

## CTA / 導線
- Primary CTA: 申し込む / 予約する / 無料体験する / 資料請求
- Secondary CTA: FAQ、価格、レビュー、比較表
- Portal CTA: `plans.html?template=lp-y&plan=standard` へ接続する想定。
