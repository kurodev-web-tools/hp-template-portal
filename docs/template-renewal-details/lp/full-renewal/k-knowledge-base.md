# LP K: Knowledge Base Renewal Detail

## 基本情報
- Category: LP
- Template ID: lp_k
- Existing path: `templates/lp/k`
- Current theme name: Knowledge Base
- Renewal decision: Full Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Knowledge Base」。features は Education, School, Academic、主要色は #047857, #ecfdf5, #ffffff。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: Knowledge Base の offer を探している見込み客
- Conversion goal: 講座申込/資料請求
- Core concept: 講座申込/資料請求 に絞ったCV型LP。1ページ1目的で、訴求、証拠、価格/条件、CTAを迷わせない。
...
- Visual hook: 講座価値、カリキュラム、申込CTA。演出よりoffer理解とCTA視認性を優先する。
- Primary actions: Conversion goal (講座申込/資料請求) に直結するCTAを1つ主役にする。
...
- Visual direction: 講座価値、カリキュラム、申込CTA
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #047857, #ecfdf5, #ffffff を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #047857, #ecfdf5, #ffffff を背景、信頼材料、CTAで役割分担する。CTA色を複数に分散させない。
...
- Audience language: Knowledge Base の offer を探している見込み客 が今申し込む理由を理解できる言葉にする。
- Required concrete details: Conversion goal (講座申込/資料請求) の条件、ベネフィット、証拠、FAQ、送信後の流れ。
- Avoid: 「今すぐ」「簡単」だけで、内容や条件が薄い状態。

## CTA / 導線
- Primary CTA: 申し込む / 予約する / 無料体験する / 資料請求
- Secondary CTA: FAQ、価格、レビュー、比較表
- Portal CTA: `plans.html?template=lp-k&plan=standard` へ接続する想定。
...
## モック画像方針
必須。first viewport + proof/pricing teaser を 1 枚作る。既存テーマ名「Knowledge Base」が残っていること、CTA が最初の画面に見えること、SPでも成立する情報量であることを確認する。
