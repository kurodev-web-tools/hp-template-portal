# Business C: Clean Renewal Detail

## 基本情報
- Category: Business
- Template ID: bus_c
- Existing path: `templates/business/c`
- Current theme name: Clean
- Renewal decision: Polish
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Clean」。features は Medical, Clean, Minimalist、主要色は #30cfd0, #ffffff, #330867。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: クリニック、医療、士業、相談業
- Conversion goal: Web予約、電話、アクセス確認を固定CTAにする。
- Core concept: 不安を減らす予約導線型サイト。診療/相談内容、受付時間、初回手順を最初に見せる。
...
- Visual hook: 白ベース、淡い青、カード型の診療案内。余白は広めだが情報は隠さない。過度な演出ではなく業種固有の写真/数値/証拠で差別化する。
- Primary actions: Conversion goal (Web予約、電話、アクセス確認を固定CTAにする) に沿うCTAを置く。
...
- Visual direction: 白ベース、淡い青、カード型の診療案内。余白は広めだが情報は隠さない。
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #30cfd0, #ffffff, #330867 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #30cfd0, #ffffff, #330867 を基調に、信頼色とCTA色を分ける。業種とズレる彩度は抑える。
...
- Audience language: クリニック、医療、士業、相談業 が安心して相談できる言葉にする。
- Required concrete details: Conversion goal (Web予約、電話、アクセス確認を固定CTAにする) に必要な資料、事例、費用目安、対応範囲。
- Avoid: 汎用的な企業理念だけで、何を依頼できるか分からない状態。

## CTA / 導線
- Primary CTA: 無料相談する / 見積もり相談する
- Secondary CTA: 資料請求、導入事例、サービス詳細
- Portal CTA: `plans.html?template=business-c&plan=standard` へ接続する想定。
