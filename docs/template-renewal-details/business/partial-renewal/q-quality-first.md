# Business Q: Quality First Renewal Detail

## 基本情報
- Category: Business
- Template ID: bus_q
- Existing path: `templates/business/q`
- Current theme name: Quality First
- Renewal decision: Partial Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Quality First」。features は Quality, Precision, Minimal Luxury、主要色は #000000, #ffffff, #e5e5e5。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 精密機器、高級プロダクト、品質保証が重要な企業
- Conversion goal: 製品相談、カタログ請求、見積もり。
- Core concept: 品質証明と仕様確認を主役にする。
...
- Visual hook: 白黒基調、製品アップ、仕様表、保証カード。過度な演出ではなく業種固有の写真/数値/証拠で差別化する。
- Primary actions: Conversion goal (製品相談、カタログ請求、見積もり) に沿うCTAを置く。
...
- Visual direction: 白黒基調、製品アップ、仕様表、保証カード。
...
## Visual System
- Color system: #000000, #ffffff, #e5e5e5 を基調に、信頼色とCTA色を分ける。業種とズレる彩度は抑える。
...
- Audience language: 精密機器、高級プロダクト、品質保証が重要な企業 が安心して相談できる言葉にする。
- Required concrete details: Conversion goal (製品相談、カタログ請求、見積もり) に必要な資料、事例、費用目安、対応範囲。
- Avoid: 汎用的な企業理念だけで、何を依頼できるか分からない状態。

## CTA / 導線
- Primary CTA: 無料相談する / 見積もり相談する
- Secondary CTA: 資料請求、導入事例、サービス詳細
- Portal CTA: `plans.html?template=business-q&plan=standard` へ接続する想定。
