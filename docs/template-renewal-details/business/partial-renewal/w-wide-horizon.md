# Business W: Wide Horizon Renewal Detail

## 基本情報
- Category: Business
- Template ID: bus_w
- Existing path: `templates/business/w`
- Current theme name: Wide Horizon
- Renewal decision: Partial Renewal
- Source audit: `docs/reviews/PORTAL_CONVERSION_AUDIT_2026-05-03.md`
- Created: 2026-05-03

## 現状の読み取り
既存テーマは「Wide Horizon」。features は Horizontal Scroll, Panorama, Minimal、主要色は #ffffff, #000000, #f0f0f0。現行の一覧カードでは見た目の違いは伝わるが、受託制作としての選択理由と申し込み導線をさらに明確にする。

## リニューアルの狙い
- Target audience: 建築、写真、風景、ギャラリー
- Conversion goal: 作品相談、展示予約、問い合わせ。
- Core concept: 横長体験を残しつつ、迷わず問い合わせへ行ける。
...
- Visual hook: PCは横スクロール、SPは縦の編集レイアウトに再構成。過度な演出ではなく業種固有の写真/数値/証拠で差別化する。
- Primary actions: Conversion goal (作品相談、展示予約、問い合わせ) に沿うCTAを置く。
...
- Visual direction: PCは横スクロール、SPは縦の編集レイアウトに再構成。
- First viewport: 誰向けのテンプレートか、何ができるか、どの行動を取るべきかを5秒で理解できる状態にする。
- Mobile: CTA、主要情報、信頼材料を先に出す。PCの演出を単純縮小しない。
- Color: 既存の #ffffff, #000000, #f0f0f0 を起点に、CTA色と装飾色を分ける。

## Visual System
- Color system: #ffffff, #000000, #f0f0f0 を基調に、信頼色とCTA色を分ける。業種とズレる彩度は抑える。
...
- Audience language: 建築、写真、風景、ギャラリー が安心して相談できる言葉にする。
- Required concrete details: Conversion goal (作品相談、展示予約、問い合わせ) に必要な資料、事例、費用目安、対応範囲。
- Avoid: 汎用的な企業理念だけで、何を依頼できるか分からない状態。

## CTA / 導線
- Primary CTA: 無料相談する / 見積もり相談する
- Secondary CTA: 資料請求、導入事例、サービス詳細
- Portal CTA: `plans.html?template=business-w&plan=standard` へ接続する想定。
